...
class CustomInfoProvider(BaseProvider):
    ...
    def kong_id(self, grouping=(8, 4, 4, 4, 12), valid_elements=tuple(string.ascii_lowercase + string.digits)):
        """
        ...
        for group in grouping:
            el += self.random_sample(elements=valid_elements, length=group)
            el += "-"
        return "".join(el)
